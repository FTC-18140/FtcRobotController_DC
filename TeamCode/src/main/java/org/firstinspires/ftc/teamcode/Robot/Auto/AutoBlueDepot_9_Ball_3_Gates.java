...
public class AutoBlueDepot_9_Ball_3_Gates extends LinearOpMode {

    @Override
    public void runOpMode() throws InterruptedException {
        Pose2d start = new Pose2d(AutoPositions.Positions.START_BLUE_DEPOT.position, Math.toRadians(-45));
        Pose2d launchPos1 = new Pose2d(AutoPositions.Positions.CLOSE_LAUNCH_ZONE_BLUE.position, Math.toRadians(180));
                                            ...
                                            new SequentialAction(
                                                    new ParallelAction(
                                                            robot.drive.actionBuilder(start)
                                                                    .strafeToSplineHeading(launchPos1.position, Math.toRadians(-180))
                                                                    .build()
                                                    ),
                                                    // Launch Preloads
                                                    robot.launchAction(),
...
//                                                    robot.waitForTime(9.5),
                                                    ...
                                                    new RaceAction(
                                                            robot.drive.actionBuilder(gatePos)
                                                                    .setReversed(true)
                                                                    .splineToConstantHeading(new Vector2d(gatePos.position.x, intakePos2.position.y + 12), Math.toRadians(-90))
                                                                    .splineToConstantHeading(intakePos2.position, Math.toRadians(90))
                                                                    .splineToConstantHeading(new Vector2d(intakePos2.position.x, 59), Math.toRadians(90), new TranslationalVelConstraint(33))
                                                                    .build(),
                                                            robot.indexerFullAction()
                                                    ),
                                                    new RaceAction(
                                                            robot.drive.actionBuilder(new Pose2d(new Vector2d(intakePos2.position.x, 59), Math.toRadians(90)))
                                                                    .setTangent(Math.toRadians(-90))
                                                                    .splineToConstantHeading(new Vector2d(intakePos2.position.x, 47), Math.toRadians(-90))
                                                                    .splineToConstantHeading(gatePos.position, Math.toRadians(90), new TranslationalVelConstraint(30))
                                                                    .build()
                                                    ),
